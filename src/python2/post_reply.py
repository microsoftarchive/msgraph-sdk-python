...
class PostReplyRequest(RequestBase):

    def __init__(self, request_url, client, options, post):
        super(PostReplyRequest, self).__init__(request_url, client, options)
        self.method = "POST"
        self.body_options={}
...
class PostReplyRequestBuilder(RequestBuilderBase):

    def __init__(self, request_url, client, post):
        super(PostReplyRequestBuilder, self).__init__(request_url, client)
        self._method_options = {}

        self._method_options["Post"] = post._prop_dict

    def request(self, options=None):
        """Builds the request for the PostReply
        ...
        Returns: 
            :class:`PostReplyRequest<microsoft.msgraph.request.post_reply.PostReplyRequest>`:
                The request
        """
        req = PostReplyRequest(self._request_url, self._client, options, self._method_options["Post"])
        return req
